...
height = 800
pg.init()
pg.display.set_caption("SnakeII")


STAT_FONT = pg.font.SysFont("Arial", 30)
...
def drawGrid(win):
    ...
    pg.draw.line(win, (10,155,0), (0,int(height/2)),(total_width,int(height/2)),2)
    for i in range(total_width//width):
        pg.draw.line(win, (10,155,0), (int(width*i),0),(int(width*i),height),2)
...
def update_win(win, snakes, snacks, gen, scores, replay, gens = None):
    ...
    if not replay:
        ...
        # поколения с максимальным количеством очков
    ...
    if replay:
        ...
        # поколение с наибольшим количество очков
        score_label = STAT_FONT.render("At Gene: " + str(genHighscore), 1, (255, 0, 0))
        win.blit(score_label, (10, 10))  
...
def run(config_file):
    ...
    #отвечает за запуск игры и количество поколений которые змейка будет обучаться
    winner = p.run(run_game, 10)
...
def run_game(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        if len(snakes) == 6:
            while len(snakes) > 0:
                if showGame:
                    pg.time.delay(50)#скорость игры
                    #clock.tick(50)
